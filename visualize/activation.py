...
def get_activation(model, layer, image, device = torch.device('cuda'), dtype = torch.float32):
    ...
    for module_pos, module in model.features._modules.items():
        if "denseblock" in module_pos:
            for module_pos_1, module_1 in module.named_modules():
                print (module_pos_1, module_1)
                if layer in module_pos_1:
                    x = module_1(x)
                    conv_output.append(x)
                elif "denselayer" in module_pos_1:
                    for module_pos_2, module_2 in module_1.named_modules():
                        if layer in module_pos_2:
                            x = module_2(x)
                            conv_output.append(x)
                        else:
                            x = module_2(x)
